...
from jss_p5b.heldout_guard import authorize_heldout, select_heldout_case
...
def _arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run one frozen P5B HELD_OUT case")
...
def _docker_command(case_id: str, run_id: str) -> list[str]:
    ...
    return [
        ...
        "--network=none", "--cpus=1",
        "-e", "JSS_P5B_HELDOUT_AUTHORIZED=YES",
        "-e", "PYTHONDONTWRITEBYTECODE=1",
        "-e", "OMP_NUM_THREADS=1",
        "-e", "OPENBLAS_NUM_THREADS=1",
        "-e", "MKL_NUM_THREADS=1",
        "-e", f"PYTHONPATH={python_path}",
        "-v", mount, "-w", "/workspace", S01_IMAGE,
        "python3", "run_p5b_heldout.py",
        "--case-id", case_id, "--run-id", run_id,
        "--execute-authorized", "--inside-container",
    ]


def main() -> int:
    args = _arguments()
    case_result_path = authorize_heldout(
        ROOT,
        case_id=args.case_id,
        run_id=args.run_id,
        cli_authorized=args.execute_authorized,
    )
    select_heldout_case(load_case_matrix(ROOT), args.case_id)
    ...
    from jss_p5b.heldout_executor import execute_heldout_case

    result = execute_heldout_case(
        ROOT,
        case_id=args.case_id,
        run_id=args.run_id,
