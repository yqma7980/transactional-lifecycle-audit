"""Separately authorized one-case runner for L2-D3.0.
...
AUTHORIZATION_ENVIRONMENT_VARIABLE = "L2_D3_EXECUTION_AUTHORIZED"
AUTHORIZATION_ENVIRONMENT_VALUE = "YES"


def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run one separately authorized L2-D3 tangent-path case."
    )
...
def _require_double_authorization(arguments: argparse.Namespace) -> None:
    ...
    if (
        ...
        raise SystemExit(
            "Execution denied: L2_D3_EXECUTION_AUTHORIZED=YES is required"
        )
...
def _write_event_log(path: Path, result: TangentPathCaseResult) -> None:
...
def _write_path_comparison(
    path: Path,
    result: TangentPathCaseResult,
) -> None:
    ...
    with path.open("w", encoding="utf-8", newline="") as stream:
        writer = csv.DictWriter(stream, fieldnames=PATH_COMPARISON_COLUMNS)
        writer.writeheader()
        writer.writerow({key: row.get(key) for key in PATH_COMPARISON_COLUMNS})
...
def _write_future_outputs(
    ...
    _write_path_comparison(comparison_path, result)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if output_root is None:
        output_root = benchmark_root / "results" / "L2_D3_tangent_path"
