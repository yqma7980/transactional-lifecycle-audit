...
from identity import AdmissionFailure, IdentityFailure, file_map, load_native, sha256, validate_spec
...
class Recorder:
    def __init__(self, stream, max_bytes=16 * 1024 * 1024):
        self.stream = stream
    ...
    def emit(self, name, fields):
        ...
        try:
            ...
            row = dict(seq=len(self.events), monotonic_ns=time.monotonic_ns(), event=name, fields=fields)
...
def run_cell(spec):
    ...
    phase = 'SPEC_IDENTITY'
    try:
        if spec.get('original_rank') != 79:
            raise AdmissionFailure('Only rank79 is available for parent admission in this batch; ranks80/94 remain held.')
        validate_spec(spec)
        ...
        identity.update(observed, pid=os.getpid(), executable=sys.executable,
                        python_version=platform.python_version(), adapter_version='native_v2')
    ...
    oracle.update(raw_trace_sha256=digest, events=count, evaluator_sha256=sha256(HERE / 'evaluator.py'),
                  adapter_version='native_v2', collector_contract=COLLECTOR_CONTRACT)
    if oracle['execution_status'] == 'COMPLETED':
