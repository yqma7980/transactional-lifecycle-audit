"""Double-locked one-case runner for independent L5-D2."""
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run one independent L5-D2 case")
...
def _authorize(args: argparse.Namespace) -> None:
    ...
    if os.environ.get("L5_D2_EXECUTION_AUTHORIZED") != "YES":
        raise SystemExit("Execution denied: L5_D2_EXECUTION_AUTHORIZED=YES is required")
    if os.environ.get("L5_D2_THREADS") != "1":
        raise SystemExit("Execution denied: L5_D2_THREADS=1 is required")
...
def main(argv: list[str] | None = None) -> int:
    ...
    output_root = args.output_root or BENCHMARK_ROOT / "results" / "L5_D2_independent_validation"
    ...
    _write_json(
        manifest_path,
        {
            ...
            "inputs": {
                "L5_D2_execution_freeze.json": _sha(BENCHMARK_ROOT / "L5_D2_execution_freeze.json"),
                "L5_D2_case_matrix.csv": _sha(BENCHMARK_ROOT / "L5_D2_case_matrix.csv"),
                "L5_D2_independence_contract.md": _sha(BENCHMARK_ROOT / "L5_D2_independence_contract.md"),
                "l5_d2_cases.py": _sha(BENCHMARK_ROOT / "src" / "l5_d2_cases.py"),
            },
            "outputs": {
                path.name: _sha(path)
