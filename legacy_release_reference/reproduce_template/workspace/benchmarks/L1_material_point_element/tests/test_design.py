...
class FrozenDesignTests(unittest.TestCase):
    def test_design_inputs_and_hashes(self) -> None:
        ...
        self.assertEqual(result["e1_case_count"], 8)
        self.assertEqual(result["e1_scenario_variant_count"], 20)
    ...
    def test_e1_is_separately_authorized_after_mp_pass(self) -> None:
        mp_freeze = (ROOT / "L1_MP_execution_freeze.json").read_text(encoding="utf-8")
        e1_freeze = (ROOT / "L1_E1_execution_freeze.json").read_text(encoding="utf-8")
        self.assertIn('"e1_authorized": false', mp_freeze)
        self.assertIn('"e1_authorized": true', e1_freeze)
        self.assertTrue((ROOT / "src" / "l1_element.py").exists())
        self.assertTrue((ROOT / "src" / "l1_e1_cases.py").exists())
