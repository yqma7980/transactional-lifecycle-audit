# Supplementary Software S1 (manuscript package v2.15.5)
...
.\.venv-feh\Scripts\python.exe -m unittest discover -s benchmarks/FEH_D0 -p test_feh_adjudication_v2_11.py
```
...
The public GitHub v2.15.5 release and the versioned Zenodo software archive contain these additions:
...
Version 2.15.5 corrects the documented FEH adjudication unit-test command to use unittest discovery from the benchmark directory. The package manifest and SHA-256 index are regenerated from the released bytes. No scientific source, reported case, equation, frozen acceptance gate, result or interpretation changes.
